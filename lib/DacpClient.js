...
/**
 * Returns true if sessionRequest is ready to be used - if returns false then 
 * sessionRequest calls will be queued up to occur only once we have connected
 */
DacpClient.prototype.sessionRequestIfReady = function(requestPath, requestQuery, callback)
{
	if (this.status != 'authenticated') callback("Not paired or iTunes not running");
	else return this.sessionRequest(requestPath, requestQuery, callback);
}
	...
	client._getHost(function(error, server) {
		if(error) {
			...
			var options = {
				...
				// disable encoding because iTunes likes to get commas in the querystring
				// particularly for the setspeakers command
				// may need to refine if we start passing album names etc
				qsStringifyOptions: {
					encode: false
				}
			};
...
DacpClient.prototype.request = function() {
	...
	rawRequestArgs.push(function(error, rawResponse) {
		if (error) requestCallback(error, null);
		else
...
DacpClient.prototype._login = function(callback) {
	...
	if (client.status == 'authenticating') { // login already in progress
		...
		console.log('setting login request' + pairingGuid);
		client.request('login', {'pairing-guid': pairingGuid}, function(error, response) {
			console.log('receive login request',error,response);
